...
class BlockedCountries:
    ...
    def print_ip_result(self) -> None:
        ...
        _user_permission: str = input(f'{COLOR_CODE["RED"]}{COLOR_CODE["RED"]}[{COLOR_CODE["RED"]}?{COLOR_CODE["RED"]}] {COLOR_CODE["RED"]}' + 
            f'Разрешите увидеть ваш IP-адрес для проверки {COLOR_CODE["DARK"]}(Да - 1 / Нет - 0): {COLOR_CODE["RED"]}').strip()
    
        sleep(1.7)
        # Доступ разрешен
        if not _user_permission or _user_permission == "1": 
            print(f'{COLOR_CODE["RED"]}{COLOR_CODE["RED"]}[%] {COLOR_CODE["RED"]}'
            f'Производиться проверка технической части..{COLOR_CODE["RED"]}')
            ...
            if ip_county in self.__country:
                print(f'{COLOR_CODE["RED"]}{COLOR_CODE["RED"]}[!] '+
                    f'{COLOR_CODE["RED"]}К сожалению, наш сервис недоступен в вашей стране {COLOR_CODE["RED"]}({ip_county}){COLOR_CODE["DARK"]}, воспользуйтесь VPN`ом для получении доступа к ресурсам.')
            ...
            if ip_data.get("status_error"):
                print(f'{COLOR_CODE["RED"]}{COLOR_CODE["RED"]}[!] '+
                    f'{COLOR_CODE["RED"]}К сожалению, у вас {COLOR_CODE["RED"]}отсутствует {COLOR_CODE["RED"]}интернет соединение')
                
                sleep(1.7)

            else:
                print(f'{COLOR_CODE["RED"]}{COLOR_CODE["RED"]}[*] {COLOR_CODE["RED"]}'
                    f'Проверка технической части для страны {COLOR_CODE["RED"]}"{ip_county}"{COLOR_CODE["RED"]}, прошла успешно!')
        ...
        # Доступ запрещен
            ...
            print(f'{COLOR_CODE["RED"]}{COLOR_CODE["RED"]}[!] '+
                f'{COLOR_CODE["RED"]}К сожалению, провести техническую проверку не удалось, {COLOR_CODE["RED"]}ЗАБЛОКИРОВАН ДОСТУП К IP.{COLOR_CODE["RED"]}')
